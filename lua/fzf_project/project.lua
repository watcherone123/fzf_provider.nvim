...
local glob = require("fzf_project.utils.globtopattern")
local path = require("fzf_project.utils.path")
local uv = vim.loop
local M = {}
...
function M.init()
  ...
  if not config.options.manual_mode then
    autocmds[#autocmds + 1] = 'autocmd VimEnter,BufEnter * ++nested lua require("fzf_project.project").on_buf_enter()'
  ...
  vim.cmd([[
    command! ProjectRoot lua require("fzf_project.project").on_buf_enter()
    command! AddProject lua require("fzf_project.project").add_project_manually()
  ]])

  autocmds[#autocmds + 1] =
    'autocmd VimLeavePre * lua require("fzf_project.utils.history").write_projects_to_history()'

  vim.cmd([[augroup fzf_project
            au!
  ]])
